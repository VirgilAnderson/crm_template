<ul class="nav nav-tabs" role="tablist">
  <li class="nav-item">
    <a class="nav-link active" data-toggle="tab" href="#lead_pane"><i class="fas fa-dollar-sign"></i> Lead</a>
  </li>
  <li class="nav-item">
    <a class="nav-link" data-toggle="tab" href="#company_pane"><i class="far fa-building"></i> Company</a>
  </li>
  <li class="nav-item">
    <a class="nav-link" data-toggle="tab" href="#task_pane"><i class="fas fa-thumbtack"></i> Tasks</a>
  </li>
  <li class="nav-item">
    <a class="nav-link" data-toggle="tab" href="#history_pane"><i class="fas fa-history"></i> History</a>
  </li>
  <li class="nav-item">
    <a class="nav-link" data-toggle="tab" href="#note_pane"><i class="far fa-sticky-note"></i> Notes</a>
  </li>
</ul><!-- .nav nav-tabs -->
